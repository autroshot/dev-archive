---
title: "Windows Terminal에 Oh My Posh 적용하기"
description: "Windows Terminal에서 PowerShell에 Oh My Posh를 적용하는 방법을 알아본다."
publishedDate: "2023-09-24"
tags: ["Windows Terminal", "PowerShell", "Oh My Posh"]
---
...
<Admonition title="Windows Terminal">
Windows Terminal은 명령줄(command line)이나 셸(shell)을 실행하는 프로그램이다. 즉 Windows Terminal에서는 명령 프롬프트(`cmd.exe`), PowerShell, Ubuntu 등을 실행할 수 있다.
</Admonition>

Windows Terminal의 설정에서 기본 프로필을 PowerShell로 설정한다. 이제 터미널을 실행하거나 새 탭을 열면 기본적으로 PowerShell이 열린다.

다음으로 앞에서 설치한 글꼴을 지정해야 한다. 설정 ➜ 프로필 ➜ PowerShell ➜ 추가 설정 ➜ 모양 ➜ 텍스트 ➜ 글꼴에서 `MesloLGM Nerd Font`를 선택한다.

![Windows Terminal 설정에서 글꼴을 지정한다.](@assets/apply-oh-my-posh-to-windows-terminal/windows-terminal-settings.png)

원한다면 추가로 색 구성표나 글꼴 크기 등을 변경할 수 있다.

## Oh My Posh 설치하기

Windows Terminal에서 다음 명령을 실행하여 Oh My Posh를 설치한다.
...
!['Oh My Posh'가 적용된 PowerShell](@assets/apply-oh-my-posh-to-windows-terminal/powershell-with-oh-my-posh.png)
...
- https://learn.microsoft.com/ko-kr/powershell/